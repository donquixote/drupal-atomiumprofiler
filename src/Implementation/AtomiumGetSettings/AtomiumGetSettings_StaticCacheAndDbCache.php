...
/**
 * @CfrPlugin("staticCacheAndDbCache", "static cache + db cache")
 */
class AtomiumGetSettings_StaticCacheAndDbCache implements AtomiumGetSettingsInterface {
  ...
  function _atomium_get_settings_flat_filtered($theme_key, $base_themes = TRUE, $value_callbacks = ['trim']) {
    ...
    if (NULL !== $cid) {
      if (isset($this->settings_flat_filtered_by_cid[$cid])) {
        return $this->settings_flat_filtered_by_cid[$cid];
      }
      if (FALSE !== $cache = cache_get('atomium_get_settings:' . $cid, 'cache')) {
        return $this->settings_flat_filtered_by_cid[$cid] = $cache->data;
      }
    }
    ...
    if (NULL !== $cid) {
      ...
      cache_set('atomium_get_settings:' . $cid, $theme_settings_flat);
    }
